...
关于程序的几点说明：
1. 程序随机生成由立方体积木块拼成的三维几何体，并在窗口下方显示该几何体的三视图，从左至右依次为：左视图、
   正视图、俯视图。按空格键在窗口正中显示该几何体，再按一次空格键重新生成新的几何体。
2. 此程序是《几何体三视图》(G422) 程序的扩充，之前的程序只能先显示几何体。程序改进后，用户可以方便地在
   先显示几何体或先显示三视图两种练习模式间进行切换。
3. 修改和新添加的代码被标记在两条横线之间。
"""
...
class Cubes():
    ...
    drawing: 是否正在绘制几何体。
    new: 下次调用 controller() 函数时，是否生成新的几何体。
