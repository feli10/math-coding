...
1. The program randomly generates an object made of cubes, then displays the three views of the
   object at the bottom of the window. From left to right, they are the left view, front view,
   and top view. Press spacebar to display the object. Press spacebar again to generate a new
   object.
2. The program is based on cubes_3view.py (G422) which can only display cubes first. This program
   can easily switch between displaying cubes first and displaying views first.
3. The revised and newly added code is marked between two lines.
"""
...
class Cubes():
    ...
    new: whether to generate new cubes next time controller() is called.
    ...
    """
    def __init__(self, show_cubes=True):
        """Constructor.
        
        show_cubes:
            True (default): display cubes first;
            False: display views first.
        """
        self.cubes = []
